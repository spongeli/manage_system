<template>
	<div>
		...
		<el-card>
			<el-row :gutter="15">
				<el-col :span="8">
					<el-input placeholder="请输入内容" v-model="searchText" clearable class="">
			...
			<el-table :data="goodsData.list" stripe border highlight-current-row>
				...
				<el-table-column label="商品首图" width="80px">
					<template slot-scope="scope">
						<el-image style="width: 60px; height: 60px" :src="getGoodsFirstImg(scope.row.goodsHeaderImg)" fit="fit"
						 :preview-src-list="scope.row.goodsHeaderImg.split(',')"></el-image>
					</template>
				</el-table-column>
			...
			<!-- 分页插件 -->
			<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="goodsData.pageNum"
			 :page-sizes="[5, 10, 20, 30]" :page-size="goodsData.pageSize" layout="total, sizes, prev, pager, next, jumper"
			 :total="goodsData.total"></el-pagination>
		</el-card>

		<!-- 修改商品信息 -->
		<el-form ref="updateGoodFormRef" :model="originalGoodsItem" :rules="originalGoodsItemRules" label-width="110px">
			<el-dialog title="修改商品信息" :visible.sync="updateGoodsDialogVisible" width="60%" :before-close="updateGoodsClose"
			 :close-on-click-modal="false" :close-on-press-escape="false">
				<el-tabs type="border-card">
					<el-tab-pane label="商品基础属性">
						<el-form-item label="商品分类" prop="cateId">
							<template slot-scope="scope">
								<el-cascader :options="cateData" v-model="originalGoodsItem.cateIds" filterable :props="{ value: 'categoryId', label: 'categoryName', expandTrigger: 'hover', checkStrictly: true }"
								 clearable></el-cascader>
							</template>
						</el-form-item>
						<el-form-item label="商品名称" prop="goodsName">
							<el-input v-model="originalGoodsItem.goodsName"></el-input>
						</el-form-item>
						<el-form-item label="商品原价格(元)">
							<el-input v-model="originalGoodsItem.goodsOriginalPrice"></el-input>
						</el-form-item>
						<el-form-item label="商品价格(元)" prop="goodsPrice">
							<el-input v-model="originalGoodsItem.goodsPrice"></el-input>
						</el-form-item>
						<el-form-item label="商品重量(g)" prop="goodsWeight">
							<el-input v-model.number="originalGoodsItem.goodsWeight"></el-input>
						</el-form-item>
						<el-form-item label="商品数量" prop="goodsCount">
							<el-input v-model.number="originalGoodsItem.goodsCount"></el-input>
						</el-form-item>
					</el-tab-pane>
					...
					<el-tab-pane label="商品详情">角色管理</el-tab-pane>
				</el-tabs>
...
<script>
	...
	export default {
		...
		methods: {
			...
			changCateIds() {
				console.log(this.originalGoodsItem.cateIds);
			}
		}
	};
</script>
