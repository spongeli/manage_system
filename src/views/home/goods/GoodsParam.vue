<template>
	<div>
		...
		<el-card class="box-card">
			<el-alert title="注意：只允许选择一级分类设置相关参数！其参数在商品分类中显示!" type="warning" show-icon :closable="false"></el-alert>
			<el-row>
				<span>选择商品分类：</span>
				<el-cascader :options="cateData" v-model="cateIds" @change="gainCateAttr" clearable :props="{ expandTrigger: 'hover', value: 'categoryId', label: 'categoryName', checkStrictly: true  }"></el-cascader>
			</el-row>
					...
					<el-table :data="dynamicList" border stripe lazy>
						...
						<el-table-column type="expand">
							<template slot-scope="scope">
								...
								<el-input ref="saveTagInput" class="input-new-tag" v-if="inputVisible" v-model="inputValue" size="small"
	...
	export default {
		...
		methods: {
			...
			gainCateAttr() {
				...
				this.$get(`/attr/${cateid}/${this.activeName}`).then(res => {
					...
					res.forEach(item => {
						...
						item.attrListMsg = item.attrList ? item.attrList.split(",") : []
					})
					console.log(res)
			...
			changAttrStatus(attr) {
				this.$post(`/attr/${attr.attrId}/update`, {
